...
- `registry.jami.studio/registry.json`, `/items/*`, `/suites/*`, `/harness/*`, and the generated workbench/suite preview routes are static files served by Cloudflare Pages.
...
- `https://registry.jami.studio/harness/` serves the local Harness workbench and its static status routes (`status.json`, `healthz.json`, `release-readiness.json`, `provider-store-observability.json`). See [Harness status routes](/harness/status-routes).
